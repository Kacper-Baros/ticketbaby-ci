...
class Page extends Admin_Controller {
  ...
  function editor($path,$width) {
    ...
    {
        $result = $this->db->order_by('order_id','ASC')->get_where('tbl_post', array('parent_id'=>$parent,'remark'=>'5'))->result();
        $return = array();
    ...
    function save() {
        ...
        if ( $this->upload->do_upload('image'))
        {
          ...
          $data['seodescription'] = $this->input->post('seodescription');
            ...
            $route['slug'] = $slug;
            $route['route'] = 'front/page/'.$order_id;
...
/*category end*/ 
        ...
        $page = $this->db->get_where('tbl_post', array('id'=>$id))->row();
        $data['parent_id'] = $_POST['parent_id'];
        $data['order_id'] = $_POST['order_id'];
        $data['title'] = $_POST['title'];
        $data['excerpt'] = $_POST['excerpt'];
      ...
      $data['seodescription'] = $this->input->post('seodescription');
        ...
        $route['slug'] = $slug;
        $route['route'] = "front/page/".$id;
        $this->db->where('id',$page->route_id);
...
function html_ordered_menu($array=0, $parent_id = 0)
    {
      $array =   $this->db->order_by('order_id','ASC')->get_where('tbl_post', array('remark'=>'5'))->result_array();
      ...
      foreach($array as $element)
      {
        if($element['parent_id']==$parent_id)
        {
            ...
            $menu_html .= '<h6>';
            if($element['status'] == '0'){
            $menu_html .=   '<a href="'.admin_url('page/enable/'.$element['id']).'" class="btn btn-danger">Unpublished</button></a>';
            }if($element['status'] == '1'){
            $menu_html .=   '<a href="'.admin_url('page/disable/'.$element['id']).'" class="btn btn-success">Published</button></a>';
            }
            $menu_html .= '</h6>';
            $menu_html .= '</div>';
            ...
            if($element['url'] != ''){
            $menu_html .= '<a title="Edit" href="'.admin_url('page/edit_link/'.$element['id']).'" class="btn btn-link btn-icon btn-xs"><i class="icon-pen"></i></a>';
            }else{
            $menu_html .= '<a title="Edit" href="'.admin_url('page/edit/'.$element['id']).'" class="btn btn-link btn-icon btn-xs"><i class="icon-pen"></i></a>';    
            }
            $menu_html .= '<a title="Remove" href="'.admin_url('page/delete/'.$element['id']).'" class="btn btn-link btn-icon btn-xs" onclick="return confirm(\'Are you sure ?\');"><i class="icon-remove"></i></a>';
            $menu_html .= '</div>';
            $menu_html .= '</div>';
