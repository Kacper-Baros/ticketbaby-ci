...
class Awards extends Admin_Controller {
    ...
    function list_events() {
        ...
        $data['events'] = $this->db->order_by('id','DESC')->get_where('tbl_events', array('event_type' => 1))->result();
        $data['title'] = 'Events';
        $data['main'] = 'award/list';
    ...
    function save() {
        ...
        $data_add['postage_status'] = $this->input->post('postate_status');
        $data_add['creditcard_status'] = $this->input->post('creditcard_status');
        /*$data_add['longitude'] = $this->input->post('longitude');
        $data_add['latitude'] = $this->input->post('latitude');*/
		//Google map edit
    ...
    function seats($id) {
        ...
        //$data['seats_list'] = $this->db->get('tbl_event_seats')->result();
    ...
    function add_seats() {/*
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            redirect(admin_url('awards/seats/' . $id));
        }
    */
	...
	function view_seats($seat_id, $event_id) {
        $data['ticket_class'] = $this->Admin_model->ticket_class();
		$data['coupons'] = $this->db->get_where('tbl_coupen', array('status' => '1', 'delete_status' => '0'))->result();
        //$data['seats_list'] = $this->db->get('tbl_event_seats')->result();
		$data['seats_list'] = $this->db->get_where('tbl_event_seats', array('event_id' => $event_id))->result();
    ...
    function update_seats() {/* 
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            redirect(admin_url('awards/seats/' . $event_id));
        }
    */
    ...
    function delete_seats($id, $eid) {
        ...
        redirect(admin_url('awards/seats/' . $eid));
    }
