...
class Gala_dinner extends CI_Controller {
    ...
    function index() {
      $data['club_sliders'] = $this->db->get_where('tbl_events',array('category_id'=>131,'recommended_carousel'=>'1'))->result();
	  $data['resulte'] = $this->db->get_where('tbl_events',array('category_id'=>131))->result();
	  $data['categories'] = $this->db->get_where('tbl_post', array('parent_id' => '0', 'remark' => '6'))->result();
      $this->load->view('gala_dinner',$data);
    }
