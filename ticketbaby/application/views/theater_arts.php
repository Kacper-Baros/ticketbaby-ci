    ...
    <div class="row">
      <div class="col-xs-12 col-sm-12 col-md-12">
        ...
        <div class="row">
		  ...
		  <?php $club_sliders = $this->db->get_where('tbl_events',array('category_id'=>130))->result();
		  if(empty($club_sliders)){
		  ?>
          ...
          <div class="col-xs-12 col-sm-9 col-md-9" style="z-index: 9;">
			<ul id="listevents">
				<?php $filter = $city = $cityid = '';
					...
					if($filter=='OK'){
						$data = $this->db->order_by('id','DESC')->get_where('tbl_events',array('category_id'=>130, 'city'=>$city))->result();
					}
					else{
						$data = $this->db->order_by('id','DESC')->get_where('tbl_events',array('category_id'=>130))->result();
					}
					$eventCount=1;
					foreach($data as $val){
				?>
		  ...
		  <?php } else{ ?>
            <div class="where_ticket">
              ...
              <form method="POST" action="">
				...
				<br>
					<?php 
					$cities = $this->db->get_where('tbl_events',array('category_id'=>130))->result();
					$cityArr = array();
					foreach($cities as $val){
