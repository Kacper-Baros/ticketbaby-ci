...
{ 
	$result = $this->db->get_where('tbl_slider', array('id'=>$_GET['update']))->result(); 
} 
...
{ 
        ...
        $aa=$this->db->delete('tbl_slider');
	    redirect(base_url() .'admin/pageslider');
} 

$row = $this->db->get_where('tbl_slider',array('status'=>'1'))->result();
...
<?php if(@$_GET['add']=="add" or isset($_GET['update']))
    ...
    <?php if(isset($_GET['update'])){ $url=admin_url('pageslider/update'); }else { $url=admin_url('pageslider/add'); }	?>
                 
                  
	<a href="<?php echo admin_url('pageslider'); ?>"><i class="fa fa-arrow-left leftarrow" aria-hidden="true">Back</i></a>
              ...
              <div class="panel-body no-padding-top">
                <div class="form-actions text-left ">
                 ...
                 <input type="hidden" name="id" value="<?php echo $_GET['update']; ?>" />
                  ...
                  <?php } ?>
                  <a href="<?php echo admin_url('pageslider'); ?>" class="btn btn-danger">Cancel</a> </div>
              </div>
            </div>
            </div>
		  </div>
...
<?php }
	...
	{
	?>
<a href="<?php echo admin_url('pageslider/?add=add'); ?>" class="btn btn-success add_new" type="button"><i class="icon-plus"></i>Add New</a> 
<!-- Media datatable -->
<div class="panel panel-default">
			  ...
			  ?>
        		<li id="item-147">
                  <div class="clearfix">
                    ...
                    <!--<div class="status">
                      <h6><a href="<?php //echo admin_url(); ?>" class="btn btn-success">Published</a></h6>
                    </div>-->
                    <div class="chat-actions"><a title="Edit" href="<?php echo admin_url(); ?>/pageslider?update=<?php echo $val->id;?>" class="btn btn-link btn-icon btn-xs"><i class="icon-pen"></i></a><a title="Remove" href="<?php echo admin_url(); ?>/pageslider?delete=<?php echo $val->id;?>" class="btn btn-link btn-icon btn-xs" onClick="return confirm('Are you sure ?');"><i class="icon-remove"></i></a></div>
                  </div>
                  <ul class="message-list droptrue ui-sortable" id="sortable">
                  </ul>
                </li>
