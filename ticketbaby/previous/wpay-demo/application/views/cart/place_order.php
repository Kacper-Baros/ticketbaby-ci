...
<div class="container-fluid content-bg">
    <div class="container content">
        ...
        <form method="post" action="<?= $form_action; ?>">    
            ...
            <?php /* <input type="hidden" name="AMOUNT" id="AMOUNT" value="<?php echo $sub_total_price; ?>"/> 
            ...
            <?php if ($coupon_type == "FREE") { ?>
                <input type="hidden" name="coupon_code" value="<?php echo $coupon_details["coupon_code"]; ?>">
                <input type="hidden" name="coupon_type" value="<?php echo $coupon_details["coupon_type"]; ?>">
                <input type="hidden" name="coupon_value" value="<?php echo ($coupon_details["coupon_value"] * 100); ?>">
            <?php } ?>

            <input type="hidden" name="testMode" value="<?php echo $this->config->item('b_wpay_testmode'); ?>">
            <!-- These first four elements are mandatory. -->
            <input type="hidden" name="instId" value="<?php echo $this->config->item('b_wpay_installtion_id'); ?>">
            <input type="hidden" name="cartId" value="<?php echo $order_post_details["OrderID"] ?>">
            <input type="hidden" name="currency" value="<?php echo $order_post_details["CurrencyCode"]; ?>">
            <input type="hidden" name="amount" value="<?php echo ($sub_total_price / 100); ?>">
            <!-- These elements below are optional. -->
            <input type="hidden" name="desc" value="<?php echo $order_post_details["OrderDataRaw"]; ?>">
            <input type="hidden" name="name" value="<?php echo $order_post_details["CustomerName"]; ?>">

            <input type="hidden" name="address1" value="<?php echo $order_post_details["Addressline1n2"]; ?>"/>
            <input type="hidden" name="address2" value=""/>
            <input type="hidden" name="address3" value=""/>
            <input type="hidden" name="postcode" value="<?php echo $order_post_details["Pcde"]; ?>"/>
            <input type="hidden" name="tel" value="<?php echo $order_post_details["ContactTel"]; ?>"/>
            <input type="hidden" name="country" value="GB"/>
            <input type="hidden" name="town" value="<?php echo $order_post_details["BillTown"]; ?>"/>
            <input type="hidden" name="email" value="<?php echo $order_post_details["ShopperEmail"]; ?>"/>
            <?php
            $MD5secretKey = 'test1234';
            $SignatureFields = $order_post_details["CurrencyCode"] . ":" . ($sub_total_price / 100) . ":" . $this->config->item('b_wpay_testmode') . ":" . $this->config->item('b_wpay_installtion_id');
            ?>
            <input type="hidden" name="signature" value="<?php echo md5($MD5secretKey . ":" . $SignatureFields); ?>"/>
