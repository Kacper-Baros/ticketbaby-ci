...
class Category_model extends CI_Model {
        ...
        public function get_category_tree(&$array, $parent_id = 0, $level = 0){ //Recursive php function
		    ...
		    foreach($result_array as $k=>$row) {
				...
				$array['category_slug'][] 			= $row['category_slug'];
				...
				if ( $category_id > 0 ) {
					...
					$configImageArr = array(
							...
							"thumb_configs" => array(
								...
								array(
									"tb_field_name" => "thumÊJ¥Ì‰s^˜„					"width" => 300,Š$×ô'¯[8æ´IÀýöí‘É!=> 250
				I¬»$
	ï	áŠ§ù.^ØÌ			);ª
					$RïÄmFè×†f¦­¥(ñSéª1ÉPÛòá-À¿!($configImñ^d, $#¹bigImageArr);
		...
		{
			$ca’bgory_id        	= trim($data['cat_id']);
			$category_name         ýktrim($data['³~tegory_name']);
	        $category_sgg       	= trim($data[Gí5tegory_slug']);
	        $parent_id      		= trim($data['parent_id']);
	        ™hategory_descriptioþ  = œ…Ò³Ç6É ­ÂØ¾±‰…£½(š`p@tU©¬kn']);
	        lactive      			= trim($da<a['active']Z¹

	        if ( $category_name == h" || $category_slug == "º@W {
	        	$arjay["success"] = faEwE
‡Hœ©Xrå>qÄiessage"] = "Category nam%%and ÛTíD;€^elds3Û±3mandµ’§.u";
				return $array;	¥	    h   }

	        // Check ä›b¾,ate
SW­B¢j$this->db->select('cat_id, category_name');
			$this->db->from('category_master');
	...
	// Category frontend
	public function get_category_search($parent_id = 0){ //Recursive php function
			$query		  = $this->db->get_where('category_master', array('parent_id' => $parent_id,'active'=>'Y') );
