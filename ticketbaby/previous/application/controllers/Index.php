...
class Index extends CI_Controller {
    ...
    public function __construct() {
        ...
        //$this->load->model(array('advertisement_video_model'));
    ...
    public function index() {
        ...
        //$data['home_page_advertisement_video'] = $this->advertisement_video_model->get_advertisement_video('advertisement_video');
...
public function newhome(){
        ...
        $this->load->view('templates/newhome-header', $data);
        $this->load->view('NewHome', $data);
        $this->load->view('templates/newhome-footer', $data);
}
    public function music() {
        ...
        //	$data['home_page_event_list'] 	= $this->event_model->get_event_list ('home');
        $data['home_page_event_list'] = $this->music_model->get_event_list('home');
    ...
    public function viewcart() {
        ...
        $data["cart_delivery_type"] = $this->session->userdata('cart_delivery_type');
        ...
        if (intval($data["cart_captcha_time_remaining"]) < 0) {
            ...
            $this->session->unset_userdata('cart_delivery_type');
        }
        ...
        $data['delivery_standard'] = $this->event_model->get_config_values('delivery_standard');
        $data['delivery_special'] = $this->event_model->get_config_values('delivery_standard');
        $data['delivery_eticket'] = $this->event_model->get_config_values('delivery_eticket');
        //print_r($data['delivery_standard'][0]['value']);exit;
    ...
    public function order() {
        ...
        $data["form_action"] = (($this->config->item('b_wpay_testing')) ? $this->config->item('b_wpay_testurl') : $this->config->item('b_wpay_productionurl'));


        /* Coupon Details */
        $this->load->model('coupon_model');
        //$coupon_details = $this->coupon_model->get_coupon_by_code($this->input->post('customer_promo_code'));
        $customer_coupon_code = $this->input->post('customer_promo_code', $data["cart_captcha_session"]['event_customer_details']['customer_promo_code']);
        $coupon_details = $this->coupon_model->get_coupon_by_code($customer_coupon_code);
        //print_r($coupon_details);//die('test');
        $coupon_type = "";
        $coupon_value = "";
        ...
        $data['delivery_special'] = $this->event_model->get_config_values('delivery_standard');
        $data['delivery_eticket'] = $this->event_model->get_config_values('delivery_eticket');
        //print_r($data['delivery_eticket'][0]['value']);exit;
        
        if(isset($data[$data["cart_delivery_type"]][0]['value'])) {
            $sub_total_price = $sub_total_price + $data[$data["cart_delivery_type"]][0]['value'];
        }
        ...
        $data["order_post_details"]["BillCountry"] = $this->config->item('b_wpay_billing_country');
        ...
        $data["order_post_details"]["ShopperLocale"] = $this->config->item('b_wpay_language');
        $data["order_post_details"]["CurrencyCode"] = $this->config->item('b_wpay_currency');
        ...
        if ($coupon_type == "F" && $seat_count == 0) {
            $sub_total_price = ($sub_total_price - $coupon_value);
        }
        ...
        $data["order_post_details"]["OrderID"] = $this->config->item('b_wpay_cart_order_key') . $UNIQ_CART_ID;  // Order - Cart Id - needs to be unique
        //- integration user details - //
        $data["order_post_details"]["PW"] = $this->config->item('b_wpay_sha_in_pass_phrase');
        $data["order_post_details"]["PSPID"] = $this->config->item('b_wpay_PSPID');
        ...
        $data["order_post_details"]["PMLISTTYPE"] = $this->config->item('b_wpay_PMLISTTYPE');
        ;
    ...
    public function thankyou() {
        ...
        if ($this->input->get('orderID')) {
            ...
            if ($data["event_id"] > 0) {
                ...
                $data['event'] = $this->music_model->get_event(FALSE, $data["event_id"]);
            }
        } else {
            redirect(base_url());
        }
    ...
    function handlePlaceOrder() {
        ...
        if (!isset($response['ORDERID']) && !isset($response['cartId'])) {
            return false;
        }

        $cartRef = isset($response['ORDERID']) ? $response['ORDERID'] : $response['cartId'];
        $response['STATUS'] = "5";
    ...
    function handleResponse() {
        ...
        $cartRef = $response['cartId'];
        $status = $response['transStatus'];
        $payId = $response['transId'];

        if ($status == 'Y') {
            if ($updateOrder) {
                $this->paymentSuccess($cartRef, $payId, $response, true);
                ...
                $this->load->view('templates/header-event-ssl', $data);
                $this->load->view('cart/thank_you', $data);
                $this->load->view('templates/footer-event-ssl', $data);
            }
        } else {
            $this->mFailReason = "Payment was cancelled. Please try again.";
            $this->mWarning = false;
            ...
            //return $this->paymentSuccess($cartRef, $payId, $response, true);
        }
        ...
        /* $this->mFailReason = "Unknown payment error occurred";
          ...
          break;
          case "93":
          $this->mFailReason = "Worldpay technical issue - please retry your payment at a later date";
          break;
          }
    ...
    public function paymentSuccess($cartRef, $payID, $response, $is_online = false) {
        ...
        if ($is_online)
            $data = array('order_id' => $cartRef, 'pay_id' => $payID, 'pay_status' => $response['transStatus'], 'pay_date' => date("Y-m-d H:i:s"),
                'pay_response' => serialize($response)
            );
        else
            $data = array('order_id' => $cartRef, 'pay_id' => $payID, 'pay_status' => $response['STATUS'], 'pay_date' => date("Y-m-d H:i:s"),
                'pay_response' => serialize($response)
            );
        ...
        $data = array(
            'cart_id' => $cart_id[1],
            'pay_id' => $payID,
            'pay_status' => ($is_online) ? $response['transStatus'] : $response['STATUS'],
            ...
            'amount' => $cart_results["price"],
            'cart_session' => $cart_results["cart_session"],
            'cart_additional_session' => $cart_results["cart_additional_session"],
            'cart_user_session' => $cart_results["cart_user_session"],
        ...
        $event_details = $this->music_model->get_event(FALSE, $data["event_id"]);
        ...
        return $this->orderConfirmation(array('email_content' => $email_content, 'cart_user_session' => $cart_user_session));
    }
    ...
    public function sendPendingPaymentEmail($cartRef) {
        $msg = "A payment was unable to be processed and is waiting for feedback from Worldpay.

Please check your Worldpay account for payment for the order. If payment has been taken, please move the order to approved from your order retrieval area.

Order cart ref: {$cartRef}
";
        $mNotificationEmails = array($this->config->item('site_admin_email'));
        foreach ($mNotificationEmails as $to) {
            mail($to, 'Worldpay - pending transaction', $msg);
        }
    }
    ...
    public function orderConfirmation($data) {
        if ($data) {
            ...
            $this->email->bcc(array('retheesh4u@gmail.com', 'dean@vtelevision.co.uk', 'saurabh22_mca41@yahoo.co.in')); /* todo need to remove once development finished */
    ...
    public function contactOrganiser() {
        $name = $this->input->post('name');
        $email = $this->input->post('email');
        $message = $this->input->post('message');
        $to_email = 'hello@vtelevision.co.uk';
        $msg = "Name: " . $name . "\n\n";
        $msg .= "Email: " . $email . "\n\n";
        $msg .= "Message: " . $message . "\n\n";
        $headers = "From: " . $email . "\r\n";
        $headers .= "Reply-To: " . $email . "\r\n";
        if (mail($to_email, "TicketBaby - Enquiry from " . $name, $msg, $headers)) {
            redirect('/');
        } else {
            redirect('/');
        }
    ...
    public function contactusform() {
        ...
        $to_email = 'crgroup2014@gmail.com';
        ...
        // send email
        if (mail("crgroup2014@gmail.com", "New Submission", $msg)) {
            redirect('index/contactus');
        } else {
            redirect('/');
        }
        ...
        /* //send mail
          $name = $fname." ".$lname;
          $this->email->from("anaszaman@gmail.com", $name);
          $this->email->to($to_email);
