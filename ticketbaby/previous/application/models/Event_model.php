...
class Event_model extends CI_Model {
    ...
    public function get_event_seats($event = 0, $ticket_class_id = 0) {
        ...
        $this->db->select('ticket_seat_master.*,  ' . $group_unit_total_sql . '
        ...
        if ($ticket_class_id > 0) {
            $this->db->where('ticket_seat_master.ticket_class_id', $ticket_class_id);
        }
    ...
    public function get_config_values($field) {
        $this->db->select('conf.*');
        $this->db->from('config conf');
        $this->db->like('conf.field', $field, 'Both');
        $query = $this->db->get();
        return $query->result_array();
    }
