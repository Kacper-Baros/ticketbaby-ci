...
class Music_model extends CI_Model {
        ...
        public function get_event($slug = FALSE, $id = FALSE)
		{
			...
			$this->db->select('music_master.*,category_event.category_id,category_master.category_slug, 
							   DATE_FORMAT(music_master.start_date,"%b") as start_date_month,
							   DATE_FORMAT(music_master.start_date,"%d") as start_date_date,
							   DATE_FORMAT(music_master.start_date,"%W") as start_date_day,
							   DATE_FORMAT(music_master.start_date,"%b %d %Y %l:%i %p") as start_date_format,
							   DATE_FORMAT(music_master.start_date,"%l:%i %p") as start_time_format'
							   );
			$this->db->from('music_master');
			$this->db->join('category_event', 'category_event.event_id = music_master.id','left');
			$this->db->join('category_master', 'category_master.cat_id = category_event.category_id','left');

			if ($id > 0) {
				$this->db->where('music_master.id', $id);
			}else{
				$this->db->where('music_master.slug', $slug);
			}
			...
			$this->db->from('music_master');
			$this->db->join('ticket_seat_master', 'ticket_seat_master.event_id = music_master.id','left');
			if ($id > 0) {
				$this->db->where('music_master.id', $id);	
			}else{
				$this->db->where('music_master.slug', $slug);	
