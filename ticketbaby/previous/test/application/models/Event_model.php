...
class Event_model extends CI_Model {
		...
		public function get_event_list($filter)
		{
			if ( $filter == "home" ) {
				...
				$this->db->select('em.title,em.slug,em.thumb1_main_carousel,em.thumb1_recommended_carousel,
								   em.show_main_carousel, em.show_recommended_carousel, em.show_hot_ticket, em.show_just_announced,em.start_date,
		...
		*/
		...
		public function get_event_by_id($event_id = 0,$comingfrom=NULL)
		{
			if ( $event_id > 0 )
			{
				if($comingfrom==''){
					...
					$this->db->select('user_event.*');
					$this->db->from('user_event');
					$this->db->where('user_event.id', $event_id);
					$query = $this->db->get();
		...
		public function updateEventThumb($data,$comingfrom) 
		{
			...
			if ( isset($data['img_extension']) ) {
				...
				'img_extension_standard_carousel' 	=> $data['img_extension_main_carousel'],
				'thumb1_main_carousel' => $data['thumb1_main_carousel'],
				'thumb1_standard_carousel' => $data['thumb1_main_carousel_2']
				);
			...
			if($data["user_event_id"]!=''){
				$this->db->where('id',$data["user_event_id"]);
		...
		***********/
		...
		public function createUpdateEvent($data) 
		{
	        ...
	        $slug       			= trim($data['slug']);
	        $start_date       	  = date("Y-m-d", strtotime(trim($data['start_date'])));
	        $end_date       		= date("Y-m-d", strtotime(trim($data['end_date'])));
			...
			$user_event_id	   = trim($data['user_event_id']);
			...
			if($this->input->get('comingfrom')!='')
				$comingfrom = $this->input->get('comingfrom');
			else
				$comingfrom = '';	
			...
			{ 
				...
				if ( $event_id < 1 || ($this->input->get('comingfrom') && ($this->input->get('is_approved')==base64_encode(0))) ) {//INSERT
					$event_data['id'] =  $this->getNextId();
					$array["created_date"]   = date("Y-m-d H:i:s");
					if($this->input->get('comingfrom') && ($this->input->get('is_approved')==base64_encode(0))){						
						$event_data['is_user_paid'] = '1';
						$paid_user_event_thumb1 = $this->input->post('paid_user_event_thumb1');
						$paid_user_event_thumb2 = $this->input->post('paid_user_event_thumb2');
						$img_extension = explode('.',$paid_user_event_thumb1);
						$event_data['img_extension'] = '.'.$img_extension['1'];
						$user_event_id = $event_id;
						$event_data['user_event_id'] = $user_event_id;	
						
						/*UPDATE USER EVENT TABLE(START)*/
						
						$userPaidEventData = array(
							//'user_id'				=> $u_id,
							'title' => $title,
							'slug' => $slug,
							'start_date'=> $start_date,
						...
						$this->db->where('id', $user_event_id);
						$this->db->update('user_event', $userPaidEventData); 
						//echo	$this->db->last_query();die;
					...
					$this->db->select('is_user_paid,img_extension');
					$this->db->from('event_master');
					$this->db->where('id', $event_id);
					$this->db->limit(1);
					$query = $this->db->get();
					$is_user_paid = $query->result_array()[0]["is_user_paid"];
					...
					if($event_id > 0){
						...
						$this->db->update('user_event', $user_event_data);
					}
					...
					/*UPDATE USER EVENT TABLE(START)*/
						...
						$userPaidEventData = array(
							...
							'additional_charity' => intval($data['additional_charity']) > 0 ? trim($data['additional_charity']) : 0, 
							'map_location' => trim($data['map_location']),
							'youtube_url' => addslashes(trim($data['youtube_url'],' ')),
							'show_main_carousel' => $show_main_carousel,
							'show_recommended_carousel' => $show_recommended_carousel,
							'show_hot_ticket' => $show_hot_ticket,
							'show_just_announced' => $show_just_announced,
					...
					/* Category event mapping  */
					$this->db->select('category_event_id');
					$this->db->from('category_event');
					$this->db->where('event_id', $event_id);
					$this->db->limit(1);
					$query = $this->db->get();
					$category_event_arr = $query->result_array();
					//echo $this->db->last_query();
					//echo "<pre>";print_r($category_event_arr);die;
					$category_data = array(
							'category_id' => $category_id
						);
						
					$this->db->where('category_event_id', $category_event_arr[0]['category_event_id']);
					$this->db->update('category_event', $category_data); 	
				...
				if ( intval($data['registered_postage_fee']) > 0 ) {
					$additional_charge_data = array(
						'additional_charge_title' => 'Registered Postage Fees',
						'additional_charge_field' => 'registered_postage_fee',
						'additional_charge_type' => $data['registered_postage_fee_type'],
						'additional_charge' => trim($data['registered_postage_fee']),
						'event_id' => $event_id,
						'views' => 'R,B,P'
					);
					$this->db->insert('event_additional_charges', $additional_charge_data);
				...
				if ( intval($data['special_postage_fee']) > 0 ) {
					$additional_charge_data = array(
						'additional_charge_title' => 'Special Postage Fees',
						'additional_charge_field' => 'special_postage_fee',
						'additional_charge_type' => $data['special_postage_fee_type'],
						'additional_charge' => trim($data['special_postage_fee']),
						'event_id' => $event_id,
						'views' => 'R,B,P'
					);
					$this->db->insert('event_additional_charges', $additional_charge_data);
				...
				if ( $event_id > 0 ) {
					if($this->input->get('comingfrom') && ($this->input->get('is_approved')==base64_encode(0))){					
						$event_data_user_paid['thumb1'] = 'user_event_img_' . $user_event_id.'_thumb1'.$query->result_array()[0]["img_extension"];
						$event_data_user_paid['thumb2'] = 'user_event_img_' . $user_event_id.'_thumb2'.$query->result_array()[0]["img_extension"];
						$this->db->where('id', $event_id);
						$this->db->update('event_master', $event_data_user_paid); 
					}else{
						...
						if ( $RESPONSE["img_thumb_arr"] && sizeof( $RESPONSE["img_thumb_arr"] ) > 0 ) {
							...
							$RESPONSE["img_thumb_arr"]["user_event_id"] = $user_event_id;
							$this->updateEventThumb ( $RESPONSE["img_thumb_arr"],$comingfrom );
						}
					...
					$configImageArr = array(
							...
							"thumb_configs" => array(
								...
								array(
									"tb_field_name" => "thumb1_main_carousel_2",
									"file_name" => "thumb2",
									"width" => 356,
									"height" => 262
								)
		...
		//DELETE FROM EVENT MASTER
		public function deleteEvent($data){
			//echo "<pre>";print_r($data);die;
			$eventId = trim($data['eventId']);
			$userEventId = trim($data['userEventId']);
			
			$this->db->where('id', $eventId);
			$this->db->delete('event_master'); 
			
			$this->db->where('id', $userEventId);
		...
		***********/
		...
		{ 
			...
			$organizer_contact_number   			= trim($data['organizer_contact_number']);	
			$organizer_email_id   			= trim($data['organizer_email_id']);
			$organizer_website   			= trim($data['organizer_website']);			
			...
			'organizer_email_id' =>$organizer_email_id,
			...
			$this->session->set_userdata('event_user', serialize($event_id));
					...
					/*CREATE THUMBNAIL FOR EVENT_MASTER IN ADVANCE(STARTS)*/
					/* Set Category Main Image */
					$configImage = array();
					...
					$configImage['max_height']      = '0';
					$configImage['overwrite']       =  TRUE;
					$configImage['file_name']       =  'user_event_img_' . $event_id; 
					...
					/*CREATE THUMBNAIL FOR EVENT_MASTER IN ADVANCE(ENDS)*/
		...
		/* paid event section */	
		public function paid_event_record_count() {			
			$this->db->from('user_event');
			$this->db->where('event_type','paid');
			$count = $this->db->count_all_results();
			//echo $this->db->last_query();
			return $count;
		}
		...
		public function uploadImages($configImage, $configImageArr) {
			...
			//echo "configImage=<pre>";print_r($configImage);
			//echo "configImageArr=<pre>";print_r($configImageArr);
            ...
            { 
				...
				//echo "uploadedFileArr=<pre>";print_r($uploadedFileArr);
				
				if($i == 1)
					$uploadedFileArr['new_image'] = '';	
				...
				foreach($configImageArr["thumb_configs"]  as $configImageArrItem) {
	                ...
	                if ( ! $this->image_lib->resize())
	                {
	                    ...
	                    {
							//echo "configImageLib=<pre>";print_r($configImageLib);
	                        chmod($configImageLib['new_image'], 0777); ## this should change the permissions
							$array["success"] = true;
							$array["message"] = "Thumb image uploaded successfully";
	...
	***/
	...
	public function get_events($limit,$start,$event_name=null)
		...
		public function get_paid_events($limit,$start)
		{
			$this->db->limit($limit, $start);
			$this->db->select('user_event.*');
			...
			$this->db->order_by('user_event.id DESC');
			$query = $this->db->get();
	...
	public function record_count($event_name=null) {
		...
		public function get_recentmost_event_created_time()
		{		
