...
class Index extends CI_Controller {
	...
	public function __construct()
	{
			...
			$this->load->model(array('advertisement_video_model','cms_category_model','page_model'));
	...
	public function viewcart() {
		...
		//echo $data["postage_type"] = $this->input->post('postage_type');die;
		$data['event'] 				= $cart_event = $this->session->userdata('cart_event');//echo "<pre>";print_r($data );die;
		...
		//echo "<pre>";print_r($data["cart_captcha_session"]);die;
	...
	public function billing() {
		...
		$data["postage_type"] = $this->input->post('postage_type');
		$data["postage_charge"] = $this->input->post($data["postage_type"].'_text_val');
		$data["subTotalPass"] = $this->input->post('subTotalPass');
		
		$data["cart_session"] = $this->session->userdata('cart_session'); 
		$data["cart_additional_session"] = $this->session->userdata('cart_additional_session');
		$data["cart_captcha_session"] = $this->session->userdata('cart_captcha_session'); 
		//echo "<pre>";print_r($data["cart_captcha_session"]);
	...
	public function order() {
		...
		/* Coupon Details */		
		$coupon_code = $data["cart_captcha_session"]["event_customer_details"]["customer_promo_code"];		
		$coupon_details  = $this->coupon_model->get_coupon_by_code($coupon_code);
		...
		$cart_user_session["user_details"] = array(
													...
													'customer_promo_code' => $coupon_code,
		...
		//echo "<pre>";print_r($cart_user_session);die;
		
		foreach ($cart_session as $k=>$cart_session_item):
			$selected_tables = sizeof($cart_session_item["selected_tables"]); //1
			$table_count = 0;
			$seat_count  = 0;
			$unit_price  = 0;
		...
		$event_additional_charges = $data['event']["event_additional_charges"];	//echo "<pre>";print_r($event_additional_charges);
		...
		for ($I=0; $I<sizeof($event_additional_charges); $I++ ) { 
			if ( $event_additional_charges[$I]["additional_charge_type"] == 'F' ) {				
				if(($event_additional_charges[$I]["additional_charge_field"]=='registered_postage_fee') || ($event_additional_charges[$I]["additional_charge_field"]=='special_postage_fee')){ 
					if($event_additional_charges[$I]["additional_charge_field"]==$data["postage_type"]){
		...
		//$actualPrice = $sub_total_price; // ARI
		
		//echo "<pre>";print_r($additional_percentage_arr);die;
		...
		$subTotalPass = str_replace(',', '', $data["subTotalPass"]);
		

		if ( $coupon_type == "F" && $seat_count==0) {
			//$sub_total_price	=	 ($sub_total_price-$coupon_value); 
			$data["subTotalPass"]	=	 number_format(($subTotalPass-$coupon_value),2); 
		}
			
		if ( $coupon_type == "P" && $seat_count==0) {
			$data["subTotalPass"]	=	number_format($subTotalPass-($subTotalPass*$coupon_value)/100,2); 
		}
		
		//$data['sub_total_price']							=	 $sub_total_price * 100; // this is 1 pound (100p)//ARI
		$data['sub_total_price']							=	 str_replace(',', '', $data["subTotalPass"]) * 100; // this is 1 pound (100p)
        ...
        "";
		//echo "<pre>";
		//print_r($data["order_post_details"]["DigestivePlain"]);die;
...
";
	...
	public function contactus(){
		$data['contact_details']       =  $this->page_model->get_page_by_id ('16');
		$data['cmsCategories'] = $this->cms_category_model->getCmsCategories();
		//echo "<pre>";
		//print_r($data);
