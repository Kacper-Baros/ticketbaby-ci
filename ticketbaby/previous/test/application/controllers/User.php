...
class User extends CI_Controller
{
		...
		****/
		...
		public function save_registration() {
			...
			//die('trest');
				if($rspn==0)
				{
					$response = $this->user_details->registerUserNew($register_value);
					if($response){
		...
		****/
		public function edit_user_detail()
		{
				...
				if($response)
				{
					$this->session->set_flashdata('success', '<div class="alert alert-success display-hide"><span>Successfully <strong></strong>Changed Profile.</span></div> ');				
		...
		****/
			...
			$url 	        				=	 base_url() .  ($this->config->item('index_page') ? $this->config->item('index_page') . "/" : "")  . "user/order_detail";
			$config['base_url']             =    $url;
			$data['url']					=	 $url;
			$config['total_rows']		    =    $this->user_details->get_orders_details($user_email);
			...
			$data['orders'] 		  =  $this->user_details->get_orders_details($user_email,$total,$start,$to_date,$from_date);
		...
		public function my_event($event_id=null){
			...
			//	print_r($data['all_event']);die('test');
			
			$this->load->view('templates/header', $data);
			$this->load->view('user/my_event');
			$this->load->view('templates/footer', $data);
		}
		
		//invite people for event
		public function invitePeople($event_id=null){
			$this->check_login();
			...
			$data['user']			= $user_order_details;
			$this->load->view('templates/header', $data);
			$this->load->view('user/invitePeople');
			$this->load->view('templates/footer', $data);
			
		}
		// event_preview_email
		public function event_preview($event_id=null){
		 
			$all_event = $this->user_details->user_event_detail($event_id);
			$data['all_event'] = $all_event;
			//print_r($data['all_event']);die;
			$this->load->view('templates/header', $data);
			$this->load->view('user/event_preview', $data);
			$this->load->view('templates/footer', $data);
			...
			if($this->input->post('send')){
				 ...
				 if($event_id){
					 ...
					 if($this->input->post('email')==''){
						...
						$config['upload_path'] = FCPATH . 'assets/upload/user/invite_csv/';
						$config['allowed_types'] = '*';
						$this->load->library('upload', $config);
						
						if (!$this->upload->do_upload('person')) { 
							...
							$file_path =  FCPATH . 'assets/upload/user/invite_csv/'.$file_data['file_name'];
	 
							if ($this->csvimport->get_array($file_path)) {
								$csv_array = $this->csvimport->get_array($file_path);
								foreach ($csv_array as $row) {
									$email.=$row['Guest\'s Email'].',';
								    $emailArr[] =  $row['Guest\'s Email'];
					...
					$all_event = $this->user_details->user_event_detail($event_id);
					$avatar		=	$all_event[0]['thumb1'];
					if(!empty($avatar)){ 					
						$url 		= 	 base_url();
						$path		=	$url . "/assets/upload/event/thumb/".$avatar;
					...
					$preview =   base_url()."index.php/user/image_preview_email/{$event_id}";
					$preview	.=  base64_encode($email)."/";
					$preview	.=  base64_encode($code)."/";
					$preview	.=  base64_encode($event_id)."/";
					$preview	.=  base64_encode($user_id)."/";
					if($response){
					...
					//print_r($html_email);die;
					  ...
					  redirect("user/invitePeople/{$event_id}");
					  }
				 }
				}else{
		...
		// event_preview_email
		public function image_preview_email($event_id=null){
		 
			$all_event = $this->user_details->user_event_detail($event_id);
			$data['all_event'] = $all_event;
			//print_r($data['all_event']);die;
			$this->load->view('templates/header', $data);
			$this->load->view('user/image_preview_email', $data);
			$this->load->view('templates/footer', $data);
		
		}
		
		// attend event 
		public function attend($id,$email,$code,$event_id,$user_id){
			
			if($this->input->post('confirm')){ 
				$invited_person_name = $this->input->post('name');
				$invited_person_contact_number = $this->input->post('contactNo');
				
				$all_event = $this->user_details->user_event_detail($id);
				$data['all_event'] = $all_event;
				//print_r($data['all_event']);	die;		
				...
				$data['invited_person_name']	=	$invited_person_name;
				$data['invited_person_contact_number']	=	$invited_person_contact_number;
				...
				$this->load->view('templates/header', $data);
				$this->load->view('user/attend_first_step', $data);
				$this->load->view('templates/footer', $data);
			}
		}
		...
		public function image_preview($user_id=null)
		{		
			$all_event = $this->user_details->user_event_image($user_id);
			$data['all_event']	=	$all_event;
			$user_order				= $this->session->userdata('user_cart');
			$user_order_details 	= unserialize($user_order);
			$data['user']			= $user_order_details;
		...
		//EXPORT INVITED PERSON's LIST (START)//ARI
		public function exportInvitedPersonDetails($event_id=null){
			$data['event_id']		=	$event_id;
			$user_order				= $this->session->userdata('user_cart');
			$user_order_details 	= unserialize($user_order);
			$data['user']			= $user_order_details;
			...
			$delimiter = ",";
			$newline = "\n";
			$query = $this->db->query("select invite_email, created_date as mail_sent_on, attend_user as is_accepted,invited_person_name,invited_person_contact_number from invite_user_event WHERE user_id='".$user_id."' AND event_id='".$event_id."'");
