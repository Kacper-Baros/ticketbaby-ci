...
class Event extends CI_Controller {
        ...
        {
                parent::__construct();
                $this->load->model(array('event_model','category_model','countries_model','states_model','cities_model'));
        }
		...
		****/
		public function Create()
		{
			...
			$data['categories'] = $this->category_model->get_categories(FALSE,'','');
			$data['countries'] = $this->countries_model->get_all_countries();
					
		
			
			
			$data['categoriesDropDown'] = $this->buildMenu($data['categories'],0);
			
			//echo $data['categoriesDropDown'];die;
		...
		public function buildMenu($options, $parent, $level=0)
		{
			$html = '';
			if($level == 0)
				$html.='<select class="categoryRecrsv form-control"  name="category" >';
			foreach ($options as $opt)
			{
			   if ($parent == $opt['parent_id'])
			   {
				   $indent = str_repeat('----', $level); 
				   $html.="<option value='".$opt['cat_id']."'>".$indent .$opt['category_name']."</option>";
				   $html.=$this->buildMenu($options, $opt['cat_id'], $level+1);
			   }
		 ...
		 public function getStatesList(){ 
			...
			$stateList = $this->states_model->get_states_by_country_id($countryId);
			$html = '';
			if(!empty($stateList)){
		 ...
		 public function getCitiesList(){ 
			...
			$cityList = $this->cities_model->get_cities_by_state_id($stateId);
			$html = '';
			if(!empty($cityList)){
	...
	{		
		...
		{ 
		 ...
		 //  print_r($this->input->post());die;
				...
				$organizer_website   = $this->input->post('organizer_website');
				$organizer_description   = $this->input->post('organizer_description');
				
				
		
			$response = $this->event_model->createUserEvent($this->input->post());
