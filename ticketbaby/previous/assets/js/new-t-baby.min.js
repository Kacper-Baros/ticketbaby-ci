...
!function(a) {
    ...
    TBABY.init = function(a) {
        ...
        a('button[name="btn-hot-ticket-tab"]').click(function() {
            ...
            if ("vvip-platinum" == b || "vip-platinum" == b)
                a("#ticketClassTooltip .modal-title-vip").show(), a("#ticketClassTooltip .modal-body-vip").show();
            else if ("gold" == b)
                a("#ticketClassTooltip .modal-title-gold").show(), a("#ticketClassTooltip .modal-body-gold").show();
            else if ("premium" == b)
                a("#ticketClassTooltip .modal-title-premium").show(), a("#ticketClassTooltip .modal-body-premium").show();
            else if ("standard" == b)
                a("#ticketClassTooltip .modal-title-standard").show(), a("#ticketClassTooltip .modal-body-standard").show();
            else if ("after-party" == b)
                a("#ticketClassTooltip .modal-title-afterparty").show(), a("#ticketClassTooltip .modal-body-afterparty").show();
            else {
                if ("after-party-ticket" != b)
                    return;
            ...
            a(".alert-error-captcha-select").hide();
			$( "button#captcha-verify" ).trigger( "click" );
			//, a("#captchaSelect").modal({
              //  show: !0
            //})
            ...
            if (quantity_all > 0) {
                ...
                $.ajax({
                    ...
                    data: from = $('form#add_cart_form').serialize() + '&section=' + section,
                    success: function(data) {
                        if (data == 2) {
                            ...
                            // AFTER SESSION CART NEW CALL TO setSessionCaptcha
                            a.ajax({
                                type: "POST",
                                url: TBABY.configs.base_url + b.setsessioncaptcha,
                                data: from = $('form#add_cart_form,form.form-event-details,form.captcha-select').serialize(), //a("form.captcha-select,form.add_cart_form").serialize(),
                                success: function(c) {
                                    window.location.href = TBABY.configs.base_url + b.viewcart;
                                },
                                error: function() {
                                    //alert("captcha empty");
                                }
            ...
            a("form.choose-table-seat input:checkbox:checked").length > 0 && ("table" == a("form.choose-table-seat input[name='ticket_section_name']").val() ? d = !0 : a("form.choose-table-seat input:checkbox:checked").each(function() {
                ...
                if ("after-party" == b || "adult" == b || "child" == b || "dinner-and-dance" == b || "dance" == b || "2" == c)
                    var e = a("form.choose-table-seat input[name='choose-ticket-quantity-" + a(this).val() + "']").val(),
                        f = a("form.choose-table-seat input[name='choose-ticket-quantity-" + a(this).val() + "']").attr("max");
                else
                    var e = a("form.choose-table-seat select[name='choose-ticket-quantity-" + a(this).val() + "']").val(),
        ...
        }), a("#choose-additional-ticket").change(function(c) {                
            if (a("form.form-event-details input:radio[name='after_party_ticket_select']:checked").length > 0) {
               // var selectControl = $("form.form-event-details input:radio[name='after_party_ticket_select']:checked").parent().find('#choose-additional-ticket');
                var  selectControl=$("#choose-additional-ticket");
				//alert($("#choose-additional-ticket").attr('event_id'));
				a.ajax({
                    type: "POST",
                    url: TBABY.configs.base_url + b.setsessioncart,
                    data: {
                        event_id: selectControl.attr('event_id'), ticket_class_id: selectControl.attr('ticket_class_id'),
                        unit_price: selectControl.attr('unit_price'), unit_min_purchase: selectControl.attr('unit_min_purchase'),
                        table_seat_count: selectControl.attr('table_seat_count'), table_price: selectControl.attr('table_price'),
                        event_ticket: selectControl.attr('event_ticket'), ticket_section_section_id: selectControl.attr('ticket_section_section_id'),
                        ticket_section_name: selectControl.attr('ticket_section_title'), ticket_class_title: selectControl.attr('ticket_class_title'),
                        ticket_selection_type: selectControl.attr('ticket_selection_type'), ticket_class_class: selectControl.attr('ticket_class_class'),
                        ticket_group: 'additional', choose_ticket_quantity: selectControl.val()
                    },
                    success: function(b) {
                        TBABY.stand_alone.loadSessionCart(), a("#ticketSelect").modal("hide"), a("html, body").animate({
                            scrollTop: a(".session-cart-list").offset().top
        ...
        }), a("form.form-event-details input:radio[name='delivery_type']").click(function(c) {
            if (a("form.form-event-details input:radio[name='delivery_type']:checked").length > 0) {
                var delivery_type = $("form.form-event-details input:radio[name='delivery_type']:checked").val();
                a.ajax({
                    type: "POST",
                    url: TBABY.configs.base_url + b.setsessioncart,
                    data: {
                        delivery_type: delivery_type, charge_type: 'delivery'
                    },
                    success: function(b) {
            ...
            a.ajax({
                type: "POST",
                url: TBABY.configs.base_url + b.setsessioncaptcha,
                data: from = $('form#add_cart_form,form.form-event-details').serialize(), //a("form.captcha-select,form.form-event-details,form.add_cart_form").serialize(),
                success: function(c) {
                    var d = a.parseJSON(c);
        ...
        }).on("success.form.bv", function(a, b) {
